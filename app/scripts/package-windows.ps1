...
$ProjectVersion = & "$BuildEnvPath/Scripts/python.exe" -c "import tomllib; print(tomllib.load(open('pyproject.toml','rb'))['project']['version'])"
if ($LASTEXITCODE -ne 0) {
...
$VersionedExeName = "$ProductName-$ArtifactVersion-windows-x64.exe"
$VersionedExePath = Join-Path $OutputPath $VersionedExeName
$ZipPath = Join-Path $RootDir "dist/$ProductName-$ArtifactVersion-windows-x64.zip"
