...
_RE_TDF_NUM = re.compile(r"[+\-]?\d+\.\d+E[+\-]\d+")
_RE_TDF_FLOAT_LINE = re.compile(r"^[\s+\-0-9.E]+$")


def _tdf_problem_label(line: str) -> str | None:
    if not line.startswith("1"):
        return None
    s = line[1:].strip()
    if not s or "entropy fit" in s.lower():
        return None
    return s


def _parse_tdf_out_problems(out_path: Path):
    """Yield (label, temp, fo, ho, so, cv) tuples parsed from a tdf.out file.

    ``cv`` is None for the 5-column (itp 1-3) tables.
    """
    lines = out_path.read_text(encoding="ascii", errors="replace").splitlines()
    label = "material"
    i = 0
    while i < len(lines):
        lab = _tdf_problem_label(lines[i])
        if lab:
            label = lab
            i += 1
            continue
        ln = lines[i]
        if "Temp (deg k)" in ln and "Fo-Ho" in ln and "So" in ln:
            i += 1
            rows = []
            while i < len(lines):
                row = lines[i].rstrip("\n")
                if not row.strip():
                    i += 1
                    continue
                if not _RE_TDF_FLOAT_LINE.match(row):
                    break
                nums = [float(x) for x in _RE_TDF_NUM.findall(row)]
                if len(nums) in (5, 6):
                    rows.append(nums)
                    i += 1
                    continue
            ...
            if rows:
                ...
                cv = [r[4] for r in rows] if len(rows[0]) == 6 else None
                yield label, temp, fo, ho, so, cv
            continue
        i += 1


def _generate_plots_from_out(wd: Path, on_log: Callable[[str], None] | None) -> None:
    """Regenerate the fallback PNG plots from the engine's tdf.out.

    Plotting is a UI/presentation concern; the numbers come from the native
    engine. Names match what the UI's tdf_parse expects.
    """
    if Figure is None or FigureCanvasAgg is None:
        return
    out_path = wd / "tdf.out"
    ...
    for idx, (label, temp, fo, ho, so, cv) in enumerate(_parse_tdf_out_problems(out_path), start=1):
        base = f"{idx:03d}_{sanitize_label(label)}"
        save_plot(plots_dir, f"{base}_free_energy", temp, fo, "TEMPERATURE", "FREE ENERGY", label, 0)
        save_plot(plots_dir, f"{base}_enthalpy", temp, ho, "TEMPERATURE", "ENTHALPY", label, 0)
        save_plot(plots_dir, f"{base}_entropy", temp, so, "TEMPERATURE", "ENTROPY", label, 0)
        if cv is not None:
            save_plot(plots_dir, f"{base}_heat_capacity", temp, cv, "TEMPERATURE", "HEAT CAPACITY", label, 0)
    _log = on_log if on_log is not None else print
    _log("TDF plots regenerated from tdf.out")
...
def run(
    ...
    *,
    ...
    """Run TDF via the bundled native engine (``abtdf``).

    The engine reads ``tdfdata`` and writes ``tdf.out``/``scoef`` in
    ``working_dir``; fallback PNG plots are regenerated from ``tdf.out``.
    """
    ...
    if rc == 0:
        try:
            ...
            (on_log if on_log is not None else print)(f"plot generation skipped: {exc}")
    return rc
...
def run_python(
    ...
    *,
    ...
    # Legacy pure-Python TDF implementation, kept as a numerical reference.
