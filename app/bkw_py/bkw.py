...
def run(
    ...
    *,
    ...
    """Run BKW via the bundled native engine (``abbkw``).

    ``cfg`` is accepted for signature compatibility but ignored: the native
    engine reads all run constants from BKWDATA (the ``novar``/``var`` block).
    """
    from bkw_py.engines import run_fixed_io
...
def run_python(
    ...
    *,
    ...
    """Legacy pure-Python BKW implementation, kept as a numerical reference."""
